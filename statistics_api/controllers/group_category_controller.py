...
from statistics_api.utils.query_maker import get_group_category_observations_between_dates_query, \
    get_groups_by_group_category_ids_query, get_aggregated_groups_by_group_category_ids_query
...
def group_category(request: WSGIRequest, group_category_canvas_id: int):
    ...
    group_categories = get_group_category_between_dates(request, group_category_canvas_id)
...
def group_category_aggregated(request: WSGIRequest, group_category_canvas_id: int):
    
    group_categories = get_group_category_between_dates(request, group_category_canvas_id)
    json_response = []
    aggregated_groups_by_group_category_ids_query = get_aggregated_groups_by_group_category_ids_query(tuple([int(g_cat.pk) for g_cat in group_categories]))
    groups = Group.objects.raw(aggregated_groups_by_group_category_ids_query)
...
def get_group_category_between_dates(request: WSGIRequest, group_category_canvas_id: int):
